...
1. Проверка валидации поля "Имя" в форме записи на курс
1. Проверка валидации поля "Телефон" в форме записи на курс
1. Проверка валидации поля "Email" в форме записи на курс
1. Проверка отправки формы с незаполненным полем "Имя"
1. Проверка отправки формы с незаполненным полем "Телефон"
1. Проверка отправки формы с незаполненным полем "Email"
...
#### Проверка валидации поля "Имя" в форме записи на курс:
- Шаг 1: Открыть страницу формы записи на курс
- Шаг 2: В поле "Имя" ввести символы, не являющиеся буквами (цифры, спецсимволы, и т.д.)

_Ожидаемый результат_: Поле "Имя" подсвечивается красным цветом, появляется сообщение об ошибке валидации, указывающее, что поле должно содержать только буквы.

#### Проверка валидации поля "Телефон" в форме записи на курс:
- Шаг 1: Открыть страницу формы записи на курс
- Шаг 2: В поле "Телефон" ввести номер телефона в некорректном формате

_Ожидаемый результат_: Поле "Телефон" подсвечивается красным цветом, появляется сообщение об ошибке валидации, указывающее, что номер телефона должен быть в формате +7XXXXXXXXXX.

#### Проверка валидации поля "Email" в форме записи на курс:
- Шаг 1: Открыть страницу формы записи на курс
- Шаг 2: В поле "Email" ввести email адрес в некорректном формате

_Ожидаемый результат_: Поле "Email" подсвечивается красным цветом, появляется сообщение об ошибке валидации, указывающее, что email адрес должен быть в формате example@example.com.

#### Проверка отправки формы с незаполненным полем "Имя":
- Шаг 1: Открыть страницу формы записи на курс
- Шаг 2: Не заполняя поле "Имя", нажать кнопку "Записаться"

_Ожидаемый результат_: Поле "Имя" подсвечивается красным цветом, появляется сообщение об ошибке валидации, указывающее, что поле обязательно для заполнения.

#### Проверка отправки формы с незаполненным полем "Телефон":
- Шаг 1: Открыть страницу формы записи на курс
- Шаг 2: Не заполняя поле "Телефон", нажать кнопку "Записаться"

_Ожидаемый результат_: Поле "Телефон" подсвечивается красным цветом, появляется сообщение об ошибке валидации, указывающее, что поле обязательно для заполнения.

#### Проверка отправки формы с незаполненным полем "Email"
- Шаг 1: Открыть страницу формы записи на курс
- Шаг 2: Оставить поле "Email" незаполненным, ввести имя и номер телефона
- Шаг 3: Нажать кнопку "Записаться"

_Ожидаемый результат_: Форма не отправляется, появляется сообщение об ошибке валидации поля "Email".
...
Оценка времени на автоматизацию тестирования может быть в диапазоне от 50 до 60 часов в зависимости от сложности приложения, количества требуемых тестов, инструментов, используемых для автоматизации, и наличия специалистов, участвующих в проекте. Необходимо учитывать возможные риски и включить запасное время на обработку возможных проблем в процессе автоматизации.
